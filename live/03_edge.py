...
def process(img, filename, args):
    ...
    elif args.process=='thresh':
        thresh(img, filename, args)
...
def canny(img, filename, args):
    ...
    save_image(canny, args.output, filename, args.file_extension)

def thresh(img, filename, args):
    #convert to gray
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # save_image(gray, args.output, filename+"-gray", args.file_extension)
    ...
    save_image(thresh, args.output, filename, args.file_extension)
...
def main():
    ...
    if os.path.isdir(args.input):
        ...
        for root, subdirs, files in os.walk(args.input):
            ...
            for file in files:
                #load file
                img = cv2.imread(os.path.join(args.input,file), cv2.IMREAD_UNCHANGED)
