...
def parse_args(): 
    ...
    parser.add_argument('-p','--process', type=str,
        default='resize',
        help='resize, crop, pad, canny, threshold, or canny (default: %(default)s)')
...
def process(img, filename, args):
    ...
    elif args.process=='threshold':
        threshold(img, filename, args)
    elif args.process=='contours':
        contours(img, filename, args)
...
def threshold(img, filename, args):
    #turn our image grayscale
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    #save_image(gray, args.output, filename+'-gray', args.file_extension)
    ...
    _,masked = cv2.threshold(blurred,175,255,cv2.THRESH_BINARY_INV)
    save_image(masked, args.output, filename+'-threshold', args.file_extension)

def contours(img, filename, args):
    #turn our image grayscale
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    #save_image(gray, args.output, filename+'-gray', args.file_extension)
    ...
    _,masked = cv2.threshold(blurred,127,255,cv2.THRESH_BINARY_INV)
    save_image(masked, args.output, filename+'-threshold', args.file_extension)
    ...
    for i in range(len(contours)):
        ...
        if(w >= 500 and h >= 500):
            ...
            save_image(crop, args.output, filename+'-crop_'+str(i), args.file_extension)
